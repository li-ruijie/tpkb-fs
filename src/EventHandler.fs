/// Mouse event handler implementing the scroll trigger state machine.
///
/// Processes mouse button down/up and move events to detect trigger patterns
/// (LR, single button, drag) and manage scroll mode transitions.
///
/// Uses a checker-chain pattern where each event passes through a list of
/// condition checkers until one returns a result (Some). This provides:
/// - Clear separation of concerns for each condition
/// - Easy addition of new checks without modifying existing code
/// - Deterministic evaluation order
///
/// Handler chains for each trigger mode:
/// - lrDown/lrUp: LR trigger (left+right simultaneous)
/// - singleDown/singleUp: Single button triggers (Middle, X1, X2)
/// - dragDown/dragUp: Drag triggers (hold and move threshold)
/// - noneDown/noneUp: Non-trigger buttons (exit scroll or pass through)
///
/// The active handlers are swapped at runtime via Volatile.Write when
/// the trigger type changes.
module EventHandler
...
let private initState () =
    lastEvent <- NonEvent
    lastResendLeftEvent.Value <- NonEvent
    lastResendRightEvent.Value <- NonEvent
    resentDownUp <- false
...
let private checkCorrectOrder me =
    isCorrectOrder (getLastResendEvent me).Value me
...
let private skipResendEventLR (me: MouseEvent): nativeint option =
    let pass () =
        ...
        (getLastResendEvent me).Value <- me
        callNextHook()
...
let private dragStart (info: HookInfo) =
    dragMoveX <- Math.Abs (info.pt.x - (fst dragStartPoint))
    dragMoveY <- Math.Abs (info.pt.y - (snd dragStartPoint))
