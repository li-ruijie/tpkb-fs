...
let private unionCaseNameCache = ConcurrentDictionary<obj, string>()

// http://stackoverflow.com/questions/1259039/what-is-the-enum-getname-equivalent-for-f-union-member
let getUnionCaseName (x:'a) =
    // Use cached value if available (avoids reflection during shutdown)
    match unionCaseNameCache.TryGetValue(box x) with
    | true, name -> name
    | false, _ ->
        try
            ...
            | case, _ ->
                let name = case.Name
                unionCaseNameCache.TryAdd(box x, name) |> ignore
                name
        with
