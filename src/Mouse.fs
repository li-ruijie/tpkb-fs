/// Mouse event types, triggers, and utility functions.
///
/// Defines discriminated unions for mouse events, clicks, and triggers:
/// - MouseEvent: All possible mouse button and move events (LeftDown, RightUp, Move, etc.)
/// - MouseClick: Click events for resending (LeftClick, MiddleClick, etc.)
/// - Trigger: Scroll trigger modes (LRTrigger, MiddleTrigger, LeftDragTrigger, etc.)
///
/// Also provides:
/// - Cached union case name reflection (prevents failures during shutdown)
/// - Active patterns for matching event types (LeftEvent, RightEvent, etc.)
/// - Trigger string parsing for loading from properties files
module Mouse
...
// Cache for union case names to avoid reflection failures during shutdown.
// Keyed by union case tag (int) per type, not by the full value, to prevent
// unbounded growth when union cases carry unique payloads (e.g. HookInfo).
let private tagReaderCache = ConcurrentDictionary<System.Type, obj -> int>()
let private unionCaseNameCache = ConcurrentDictionary<System.Type * int, string>()

// http://stackoverflow.com/questions/1259039/what-is-the-enum-getname-equivalent-for-f-union-member
let getUnionCaseName (x:'a) =
    try
        let t = typeof<'a>
        let reader = tagReaderCache.GetOrAdd(t, System.Func<System.Type, obj -> int>(fun t -> FSharpValue.PreComputeUnionTagReader(t)))
        let key = (t, reader(x))
        match unionCaseNameCache.TryGetValue(key) with
        | true, name -> name
        | false, _ ->
            match FSharpValue.GetUnionFields(x, t) with
            | case, _ ->
                let name = case.Name
                unionCaseNameCache.TryAdd(key, name) |> ignore
                name
    with
    | _ -> "Unknown" // Fallback during shutdown when reflection fails
