...
///
/// Contains all Win32 API imports required for tpkb's core functionality:
...
/// lParam: Pointer to MSLLHOOKSTRUCT (nativeint; only valid when nCode >= 0)
type LowLevelMouseProc = delegate of nCode: int * wParam: nativeint * lParam: nativeint -> nativeint
...
extern nativeint CallNextHookExM(nativeint hhk, int32 nCode, nativeint wParam, nativeint lParam)

/// Passes hook information to the next hook in the chain (keyboard version).
[<DllImport("user32.dll", EntryPoint = "CallNextHookEx", SetLastError = true)>]
extern nativeint CallNextHookExK(nativeint hhk, int32 nCode, nativeint wParam, nativeint lParam)
...
/// SystemParametersInfo: get/set Filter Keys settings.
let SPI_GETFILTERKEYS = 0x0032u
let SPI_SETFILTERKEYS = 0x0033u

/// FILTERKEYS structure for SystemParametersInfo.
[<Struct; StructLayout(LayoutKind.Sequential)>]
type FILTERKEYS =
    val cbSize      : uint32
    val dwFlags     : uint32
    val iWaitMSec   : uint32
    val iDelayMSec  : uint32
    val iRepeatMSec : uint32
    val iBounceMSec : uint32

    new(size, flags, wait, delay, repeat, bounce) =
        { cbSize = size; dwFlags = flags; iWaitMSec = wait; iDelayMSec = delay; iRepeatMSec = repeat; iBounceMSec = bounce }

let FKF_FILTERKEYSON = 0x00000001u

/// SystemParametersInfo: get/set keyboard repeat delay and speed.
let SPI_GETKEYBOARDDELAY = 0x0016u
let SPI_SETKEYBOARDDELAY = 0x0017u
let SPI_GETKEYBOARDSPEED = 0x000Au
let SPI_SETKEYBOARDSPEED = 0x000Bu
...
/// Layout matches C RAWMOUSE union: usFlags at 0, {usButtonFlags,usButtonData} at 4.
[<Struct; StructLayout(LayoutKind.Explicit)>]
type RAWMOUSE =
    [<FieldOffset(0)>]  val usFlags            : uint16
    [<FieldOffset(4)>]  val usButtonFlags      : uint16
    [<FieldOffset(6)>]  val usButtonData       : uint16
    [<FieldOffset(8)>]  val ulRawButtons       : uint32
    [<FieldOffset(12)>] val lLastX             : int
    [<FieldOffset(16)>] val lLastY             : int
    [<FieldOffset(20)>] val ulExtraInformation : uint32
...
extern uint32 RegisterWindowMessageW(string lpString)

/// Posts a message to the message queue of the specified window.
[<DllImport("user32.dll", SetLastError = true)>]
extern bool PostMessageW(nativeint hWnd, uint32 Msg, nativeint wParam, nativeint lParam)

/// Sets the DPI awareness context for the current process.
...
extern bool OpenProcessToken(nativeint ProcessHandle, uint32 DesiredAccess, nativeint& TokenHandle)

/// Retrieves token information. TokenElevation (class 20) returns a 4-byte int (nonzero = elevated).
[<DllImport("advapi32.dll", SetLastError = true)>]
extern bool GetTokenInformation(nativeint TokenHandle, int TokenInformationClass, nativeint TokenInformation, uint32 TokenInformationLength, uint32& ReturnLength)

// ============================================================================
// Process Enumeration (Toolhelp32)
// ============================================================================

/// Process entry from a Toolhelp32 snapshot.
...
/// Creates a snapshot of processes. dwFlags=0x2 for TH32CS_SNAPPROCESS.
[<DllImport("kernel32.dll", SetLastError = true)>]
extern nativeint CreateToolhelp32Snapshot(uint32 dwFlags, uint32 th32ProcessID)

/// Retrieves the first process from a snapshot.
[<DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Unicode)>]
extern bool Process32FirstW(nativeint hSnapshot, PROCESSENTRY32W& lppe)

/// Retrieves the next process from a snapshot.
[<DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Unicode)>]
extern bool Process32NextW(nativeint hSnapshot, PROCESSENTRY32W& lppe)
