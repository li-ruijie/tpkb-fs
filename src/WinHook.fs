...
/// any application receives them, allowing tpkb to:
...
let callNextMouseHook nCode wParam (lParam: nativeint) =
    let hhk = Volatile.Read(mouseHhk)
    WinAPI.CallNextHookExM(hhk, nCode, wParam, lParam)

let callNextKeyboardHook nCode wParam (lParam: nativeint) =
    let hhk = Volatile.Read(keyboardHhk)
    WinAPI.CallNextHookExK(hhk, nCode, wParam, lParam)
